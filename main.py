...
# ============================================================
# Telegram Business Shop Bot v9.9 Delayed Natural Flow
...
SECOND_REPLY_DELAY_SECONDS = int(os.environ.get("SECOND_REPLY_DELAY_SECONDS", "60"))

FIRST_CUSTOMER_REPLY = "سلام، بله چه محصولی می‌خوایید؟"
...
_ACTIVE_CUSTOMER_MESSAGE_ID = None
...
def state_explanation(state):
    mapping = {
        ...
        "await_size_simple": "الان فقط سایزت رو لازم دارم. اگه نمی‌دونی، قد و وزنت رو بفرست تا راهنماییت کنم.",
        ...
        "edit_menu": "بگو کدوم بخش سفارش اصلاح بشه: سایز، نام، موبایل یا آدرس.",
...
def resume_prompt(state, chat_id):
    ...
    if state == "await_size_simple":
        return "برای ادامه سایزت رو بگو؛ اگر نمی‌دونی قد و وزنت رو بفرست."
    ...
    if state == "edit_menu":
        return "برای ویرایش یکی رو بگو: سایز، نام، موبایل یا آدرس."
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS chats(
            ...
            opening_stage INTEGER DEFAULT 0,
            availability_due INTEGER DEFAULT 0,
            availability_message_id INTEGER DEFAULT 0,
            price_page_told INTEGER DEFAULT 0
        )
    """)
    ...
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS processed_customer_messages(
            business_connection_id TEXT NOT NULL,
            chat_id INTEGER NOT NULL,
            message_id INTEGER NOT NULL,
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS customer_message_replies(
            ...
            PRIMARY KEY(business_connection_id, chat_id, source_message_id),
            UNIQUE(chat_id, reply_hash)
        )
    """)
...
def claim_customer_message(connection_id, chat_id, message_id):
    """Return False when Telegram delivers an already processed message again."""
    if not message_id:
        return True
    conn = db()
    try:
        conn.execute(
            "INSERT INTO processed_customer_messages("
            "business_connection_id,chat_id,message_id,processed_at) VALUES(?,?,?,?)",
            (
                str(connection_id), int(chat_id), int(message_id),
...
def send_business(connection_id, chat_id, text, source_message_id=None, allow_repeat=False):
    ...
    reply_hash = hashlib.sha256(clean_text.encode("utf-8")).hexdigest()
    if source_id:
        conn = db()
        already_for_message = conn.execute(
            "SELECT 1 FROM customer_message_replies "
            "WHERE business_connection_id=? AND chat_id=? AND source_message_id=?",
            (str(connection_id), int(chat_id), int(source_id)),
        ).fetchone()
        repeated_text = conn.execute(
            "SELECT 1 FROM customer_message_replies WHERE chat_id=? AND reply_hash=?",
    ...
    _remember_card_message(connection_id, chat_id, result, clean_text)

    if source_id:
        conn = db()
        try:
            conn.execute(
                "INSERT OR IGNORE INTO customer_message_replies("
                "business_connection_id,chat_id,source_message_id,reply_hash,reply_text,sent_at"
                ") VALUES(?,?,?,?,?,?)",
                (
                    str(connection_id), int(chat_id), int(source_id), reply_hash,
                    clean_text, datetime.now().isoformat(timespec="seconds"),
                ),
            )
            conn.commit()
        finally:
...
def start_order(chat_id):
    """Start the simplified checkout without asking quantity or product name."""
    c = get_chat(chat_id)
    saved_photo = (c["pending_product_photo"] or "") if c else ""
    clear_cart(chat_id)
    update_chat(
        chat_id,
        ...
        cancel_return_state="",
        edit_return_state="",
        pending_product_photo=saved_photo,
    )


def simple_order_review_text(chat_id):
    c = get_chat(chat_id)
    return (
        "لطفاً اطلاعات ثبت سفارش رو یک‌بار بررسی کن 👇\n\n"
        f"سایز: {c['size']}\n"
...
def create_simple_order(chat_id):
    """Create an order without saving a product name or an invented product price."""
    c = get_chat(chat_id)
    conn = db()
    cur = conn.execute(
        """INSERT INTO orders(chat_id,product,size,full_name,phone,address,price,created_at)
           VALUES(?,?,?,?,?,?,?,?)""",
        (
            chat_id, "", c["size"], c["full_name"], c["phone"], c["address"], 0,
            datetime.now().isoformat(timespec="seconds"),
        ),
    )
    conn.commit()
...
def is_price_missing_followup(text):
    n = normalize_text(text)
    phrases = [
        "نبود", "نیست", "نذاشتین", "نگذاشتین", "پیدا نکردم", "ندیدم",
        "قیمت نبود", "قیمت نیست", "نوشته نشده", "نزده", "نزدین",
    ]
    return any(normalize_text(p) in n for p in phrases)


def schedule_availability_reply(connection_id, chat_id, source_message_id):
    due = int(time.time()) + max(1, SECOND_REPLY_DELAY_SECONDS)
    update_chat(
        chat_id,
        opening_stage=2,
        availability_due=due,
        availability_message_id=int(source_message_id or 0),
        state="opening_wait",
    )


def process_due_opening_replies():
    """Send delayed second replies without blocking other customers."""
    now = int(time.time())
    conn = db()
    rows = conn.execute(
        "SELECT chat_id,connection_id,availability_message_id FROM chats "
        "WHERE opening_stage=2 AND availability_due>0 AND availability_due<=?",
        (now,),
    ).fetchall()
    conn.close()
    for row in rows:
        send_business(
            row["connection_id"], int(row["chat_id"]), SECOND_CUSTOMER_REPLY,
            source_message_id=int(row["availability_message_id"] or 0),
        )
        start_order(int(row["chat_id"]))
        update_chat(
            int(row["chat_id"]), opening_stage=3,
            availability_due=0, availability_message_id=0,
        )
...
def checkout_prompt(chat_id, state):
    ...
    if state == "confirm_order_simple":
        return simple_order_review_text(chat_id)
...
def handle_business_message(msg, business_owner_id=0):
    ...
    if not claim_customer_message(connection_id, chat_id, message_id):
        return
    _ACTIVE_CUSTOMER_MESSAGE_ID = message_id or None
    ...
    # Fixed opening flow. The first message is answered immediately. The second
    # is consumed and receives its availability answer after one minute.
    opening_stage = int(c["opening_stage"] or 0) if c else 0
    if opening_stage == 0:
        if msg.get("photo"):
            update_chat(chat_id, pending_product_photo=msg["photo"][-1]["file_id"])
        update_chat(chat_id, opening_stage=1)
        send_business(connection_id, chat_id, FIRST_CUSTOMER_REPLY)
        return
    if opening_stage == 1:
        if msg.get("photo"):
            update_chat(chat_id, pending_product_photo=msg["photo"][-1]["file_id"])
        schedule_availability_reply(connection_id, chat_id, message_id)
        return
    if opening_stage == 2:
        # Extra messages during the one-minute wait are recorded as processed,
        # but do not receive duplicate or premature replies.
        if msg.get("photo"):
            update_chat(chat_id, pending_product_photo=msg["photo"][-1]["file_id"])
        return
    ...
    if (
        state in {"await_product_count", "await_item_name", "await_item_photo", "confirm_cart", "await_add_count"}
        or (state in {"await_size", "confirm_size"} and not cart_items(chat_id))
    ):
        start_order(chat_id)
        c = get_chat(chat_id)
        state = "await_size_simple"
    ...
    if msg.get("photo"):
        ...
        if state == "await_size_simple":
            ...
            send_business(
                connection_id, chat_id,
                "عکس محصول ثبت شد ✅ حالا سایزت رو بفرست؛ S تا 4XL. اگر نمی‌دونی قد و وزنت رو بفرست."
            )
    ...
    # Store policy for price questions: never invent or quote a price in chat.
    # If the customer says it was not there after being directed to the page,
    # answer only with the requested follow-up.
    c = get_chat(chat_id)
    if int(c["price_page_told"] or 0) and is_price_missing_followup(text):
        send_business(connection_id, chat_id, PRICE_MISSING_REPLY)
        return
    direct_price_question = (
        intent == "price"
        or "price" in core_intents
        or bool(re.search(r"(?:قیمت|قيمت|قیمط|چنده|چند تومن|چقدر)", ntext))
    )
    if direct_price_question and intent not in {"price_difference", "low_price", "wholesale_price"}:
        update_chat(chat_id, price_page_told=1)
        send_business(connection_id, chat_id, PRICE_PAGE_REPLY)
        return
    ...
    if state == "edit_menu":
        ...
        if any(x in ntext for x in ["محصول", "سبد", "کالا", "سایز"] ) or extract_explicit_size(text):
            update_chat(chat_id, state="await_size_simple", size="", pending_size="")
            send_business(connection_id, chat_id, "حتماً، سایز درست رو بفرست؛ S تا 4XL. اگر نمی‌دونی قد و وزنت رو بفرست.")
            ...
            send_business(connection_id, chat_id, "کدوم بخش ویرایش بشه؟ «سایز»، «نام»، «موبایل» یا «آدرس».")
        return
    ...
    if intent == "dont_know":
        if state in {"await_size", "await_size_simple"}:
    ...
    if intent == "size" and state not in {"await_size", "await_size_simple", "await_height_weight", "confirm_size"}:
        product_entry_states = {"await_product_count", "await_item_name", "await_item_photo", "await_add_count"}
        if state not in product_entry_states or looks_like_question(text):
    ...
    if state == "confirm_order_simple":
        if not is_affirmative_choice(text):
            if is_negative_choice(text):
                update_chat(chat_id, state="edit_menu")
                send_business(connection_id, chat_id, "باشه؛ کدوم بخش رو اصلاح کنیم؟ سایز، نام، موبایل یا آدرس؟")
            else:
    ...
    if state == "await_size_simple":
        ...
        if h and w:
            ...
            if rec:
                update_chat(chat_id, size=rec, state="await_name", pending_size="")
                send_business(connection_id, chat_id, ans + "\n\nحالا اسم و فامیلی تحویل‌گیرنده رو بفرست.")
            else:
                send_business(connection_id, chat_id, ans)
            return
        ...
        if chosen:
            update_chat(chat_id, size=chosen, state="await_name", pending_size="")
            send_business(connection_id, chat_id, f"سایز {chosen} ثبت شد ✅ حالا اسم و فامیلی تحویل‌گیرنده رو بفرست.")
            return

        send_business(
            connection_id, chat_id,
            "برای ثبت سفارش سایزت رو بفرست: S، M، L، XL، 2XL، 3XL یا 4XL. اگر نمی‌دونی قد و وزنت رو بفرست."
        )
        return
    ...
    if looks_like_product_reference(text):
        ...
        send_business(
            connection_id, chat_id,
            "بله موجوده ✅ برای ثبت سفارش سایزت رو بفرست؛ S تا 4XL. اگر نمی‌دونی قد و وزنت رو بفرست."
        )
        return
    ...
    if intent == "order":
        ...
        send_business(
            connection_id, chat_id,
            "حتماً 🌹 برای ثبت سفارش سایزت رو بفرست؛ S تا 4XL. اگر نمی‌دونی قد و وزنت رو بفرست."
        )
        return
    ...
    # Unknown messages are intentionally left unanswered. Guessing here makes
    # the account feel robotic and can send irrelevant replies.
    return
...
def main():
    ...
    print("Telegram Business Shop Bot v9.9 is running...", flush=True)

    while True:
        try:
            process_due_opening_replies()
            updates = api("getUpdates", {
                "offset": offset,
                # Short polling keeps the scheduled one-minute reply accurate
                # without blocking the bot or using a fragile sleep inside chat handling.
                "timeout": 5,
