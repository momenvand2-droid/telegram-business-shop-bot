...
# ----- Admins -----
raw_admins = os.getenv("ADMIN_IDS", "1669180525").strip()     #raw_admins = os.getenv("ADMIN_IDS", "admine 1,admine 2, ...").strip()
...
async def price_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if choice == "price_unknown":
        await query.message.reply_text(
            "سلام دوست عزیز، ممنون از پیام‌تون 🙏❤️\n"
            "برای اینکه راحت‌تر بتونید قیمت‌ها رو چک کنید، راهنمای زیر رو براتون گذاشتیم:\n"
            "📌 پست‌ها: قیمت تمامی اجناس به تفکیک داخل کپشن (توضیحات پست) نوشته شده است.\n"
            "📌 استوری‌ها: قیمت روی خودِ استوری درج می‌شود. اگر برای محصولی قیمت روی استوری نبود، لطفاً چند استوری قبل‌تر را نگاه کنید؛ معمولاً قیمت پایه برای یک دسته‌بندی مشخص ثبت شده است (مثلاً اگر قیمت یک مدل خاص مثلاً جاگر قرمز ۱,۰۰۰,۰۰۰ تومان درج شده باشد، ۱۰ مدل مشابه بعدی هم دقیقاً همان قیمت را دارند).\n"
            "از همراهی و صبوری شما صمیمانه سپاسگزاریم! ✨🛍️"
        )
    else:
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        # Important for Business chats: while an order is active, later customer
        # messages must continue the current state instead of re-triggering the
        # BUSINESS_MESSAGE entry point and restarting from NAME.
        allow_reentry=False,
    )
