...
AVAILABLE_REPLY = "بله تمامی محصولات ما موجوده ✅"
...
def handle_business_message(msg: Dict[str, Any]) -> None:
    ...
    # Receipt image/document -> send to admin, then confirmation to customer.
    ...
    if not pending.get(state_key):
        send_business(connection_id, chat_id, AVAILABLE_REPLY)
        pending[state_key] = True
        save_data(data)
        return

    # Next text after availability => send all three messages.
