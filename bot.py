...
SHIPPING_MESSAGE = """⏳ زمان آماده‌سازی و ارسال

زمان تحویل مرسوله 5 روز کاری هست و بین 1 تا 2 روز هم آماده‌سازی و ارسال محصول‌تون زمان می‌بره. 🚚
بعد از واریز لطفاً عکس رسید رو همینجا بفرستید 🌹"""
...
DELIVERY_REPLIES = [
    "⏳ زمان تحویل مرسوله حدود ۵ روز کاریه و آماده‌سازی/ارسال هم معمولاً ۱ تا ۲ روز زمان می‌بره. 🚚",
    "معمولاً مرسوله حدود ۵ روز کاری به دستتون می‌رسه و آماده‌سازی سفارش هم ۱ تا ۲ روز زمان می‌بره 🌹",
]
PING_REPLIES = [
    "بله در خدمتم 🌹",
    "بله عزیز، هستم 🌱",
...
ORDER_ALREADY_SENT_REPLIES = [
    "فرم ثبت سفارش و اطلاعات واریز بالاتر براتون ارسال شده 🌹 بعد از تکمیل فرم و واریز، عکس رسید رو همینجا بفرستید.",
    "اطلاعات ثبت سفارش بالاتر ارسال شده ✅ فرم رو تکمیل کنید و بعد از واریز عکس رسید رو بفرستید.",
]
...
DELIVERY_WORDS = ["کی به دستم", "کی میرسه", "کی می‌رسه", "چند روزه", "زمان ارسال", "زمان تحویل", "چقدر طول", "ارسال چقدر", "تحویل"]
PING_WORDS = ["الو", "هستی", "هستین", "جواب میدی", "جواب می‌دی", "سلام؟"]
...
def next_variant(state: Dict[str, Any], key: str, choices: List[str]) -> str:
    """Cycle reply variants so repeated customer pings do not look robotic."""
    counters = state.setdefault("variant_counters", {})
    idx = int(counters.get(key, 0))
    counters[key] = idx + 1
...
def classify_intent(text: str) -> Optional[str]:
    t = normalize(text)
    if is_size_request(t):
        return "size"
    if contains_any(t, PRICE_WORDS):
        return "price"
    if contains_any(t, DELIVERY_WORDS):
        return "delivery"
    if contains_any(t, PING_WORDS):
        return "ping"
    if is_order_request(t):
        return "order"
    if contains_any(t, RECEIPT_WORDS):
        return "receipt"
    return None


def reply_known_intent(connection_id: str, chat_id: int, state: Dict[str, Any], message_id: int, intent: Optional[str], stage: str) -> bool:
    """Answer normal customer questions even while an order is in progress."""
    if intent == "size":
    ...
    if intent == "price":
        ...
        return True
    if intent == "delivery":
        send_state_reply(connection_id, chat_id, state, message_id, next_variant(state, "delivery", DELIVERY_REPLIES))
        return True
    if intent == "ping":
        send_state_reply(connection_id, chat_id, state, message_id, next_variant(state, "ping", PING_REPLIES))
        return True
    if intent == "receipt" and stage == "awaiting_receipt":
        send_state_reply(connection_id, chat_id, state, message_id, next_variant(state, "receipt", RECEIPT_REMINDERS))
        return True
    if intent == "order" and stage == "awaiting_receipt":
        send_state_reply(connection_id, chat_id, state, message_id, next_variant(state, "order_again", ORDER_ALREADY_SENT_REPLIES))
        return True
    return False


def send_order_sequence(connection_id: str, chat_id: int, state: Dict[str, Any], data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Send the user's three order messages as three separate Telegram messages."""
    send_once(connection_id, chat_id, state, "order_form", ORDER_FORM)
    payment_msg = send_once(connection_id, chat_id, state, "payment", data.get("payment_message", DEFAULT_PAYMENT_MESSAGE))
    send_once(connection_id, chat_id, state, "shipping", SHIPPING_MESSAGE)
    return payment_msg
...
def process_due_availability_replies() -> None:
    ...
    for state_key, raw_state in list(pending.items()):
        ...
        state["step"] = "available"
        deferred = state.get("deferred_intent")
        if deferred in ("size", "price", "delivery", "ping"):
            # Use message_id=0 so this is an additional reply after the delayed availability reply.
            if deferred == "size":
                send_business(connection_id, chat_id, SIZE_GUIDE)
            elif deferred == "price":
                send_business(connection_id, chat_id, PRICE_REPLY)
            elif deferred == "delivery":
                send_business(connection_id, chat_id, next_variant(state, "delivery", DELIVERY_REPLIES))
            elif deferred == "ping":
                send_business(connection_id, chat_id, next_variant(state, "ping", PING_REPLIES))
...
def handle_business_message(msg: Dict[str, Any], grouped_messages: Optional[List[Dict[str, Any]]] = None) -> None:
    """Conversation flow: one delayed availability reply, but no locked/deaf states."""
    data = load_data()
    if is_business_owner_message(msg, data):
        return
    ...
    # First customer turn always opens the conversation.
    if stage == "new":
    ...
    # Second customer turn keeps the requested one-minute availability delay.
    # If that same message also contains a useful question, remember it and answer
    # immediately after the delayed "available" reply instead of dropping it.
    if stage == "waiting_second_message":
        ...
        if intent in ("size", "price", "delivery", "ping"):
            state["deferred_intent"] = intent
        pending[state_key] = state
        save_data(data)
        return

    # While the one-minute reply is pending, later customer questions are still answered.
    if stage == "availability_pending":
        if is_voice:
            send_state_reply(connection_id, chat_id, state, message_id, VOICE_REPLY)
        elif reply_known_intent(connection_id, chat_id, state, message_id, intent, stage):
            pass
        # Product photos are silently accumulated here so 10 photos don't create 10 replies.
        pending[state_key] = state
        save_data(data)
        return

    # Receipt photo only counts as receipt after payment details were actually sent.
    if image_ids:
        if stage == "awaiting_receipt":
            forwarded = False
            for item in media_messages:
            ...
            # A product photo can arrive at any time; acknowledge the batch once.
    ...
    # Even after the form/payment messages are sent, keep answering normal questions.
    if reply_known_intent(connection_id, chat_id, state, message_id, intent, stage):
        pending[state_key] = state
        save_data(data)
        return

    if stage == "completed":
        # Completed orders stay closed; unrelated messages are intentionally ignored.
        pending[state_key] = state
        save_data(data)
        return

    if stage == "available" and intent == "order":
        payment_msg = send_order_sequence(connection_id, chat_id, state, data)
        if payment_msg:
            data.setdefault("payment_messages", []).append({
                "business_connection_id": connection_id,
                "chat_id": chat_id,
